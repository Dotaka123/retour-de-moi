...
async def view_order_keys_callback(query, order_id, user_id):
    ...
    if not keys_raw:
        ...
        return
    keys = keys_raw.split("\n")
    usd = ngn_to_usd(order["total_cost"])
    text = tx["view_keys_title"].format(order["product_name"], order["quantity"], fmt_usd(usd))
...
async def process_buy_callback(query, product_id, quantity, user_id, context):
    ...
    for page in range(1, 20):
        products_result = await api_client.get_products(page=page)
        if not products_result["success"]:
            break
        results = products_result["data"]["results"]
        if not results:
            break
        for p in results:
            if p["id"] == product_id:
    ...
    saved = db.create_order(
        user_id, product_id, product["name"], quantity, total_cost_usd,
        order_data.get("keys", []), order_data.get("order_id")
    )
    if not saved:
        logger.error("Failed to save order for user %d product %d", user_id, product_id)

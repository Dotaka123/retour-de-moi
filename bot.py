    ...
    try:
        ...
        with conn.cursor() as cur:
            cur.execute(
                ...
                (user_id, product_id, product["name"], quantity, float(total_cost_usd), keys_str, order_api_id)
            )
        conn.commit()
        order_saved = True
        logger.info("Order saved OK for user %d (id=%s)", user_id, order_api_id)
    except Exception as e:
        logger.error("ORDER SAVE FAILED: user=%s product=%s name=%s qty=%s cost=%s keys=%s api_id=%s error=%s",
                     user_id, product_id, product.get('name'), quantity, total_cost_usd, len(order_keys) if order_keys else 0, order_api_id, e)
    finally:
        if conn:
            try:
                db.return_db(conn)

...
T_PROXY_PRICES = "bot_proxy_prices"
T_PROXY_ORDERS = "bot_proxy_orders"
...
def init_db():
    ...
    try:
        with conn.cursor() as cur:
            ...
            """)
            cur.execute(f"""
                CREATE TABLE IF NOT EXISTS bot_product_overrides (
                    product_id INT PRIMARY KEY,
                    price_usd DECIMAL(12,4) DEFAULT 0,
                    stock INT DEFAULT -1,
                    custom_price TINYINT DEFAULT 0,
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP
                )
            """)
            cur.execute(f"""
                CREATE TABLE IF NOT EXISTS {T_PROXY_PRICES} (
                    pkg_id INT NOT NULL,
                    days INT DEFAULT 0,
                    hours INT DEFAULT 0,
                    ...
                    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
                    PRIMARY KEY (pkg_id, days, hours)
                )
            """)
            cur.execute(f"""
                CREATE TABLE IF NOT EXISTS {T_PROXY_ORDERS} (
                    id INT AUTO_INCREMENT PRIMARY KEY,
                    user_id BIGINT NOT NULL,
...
def create_order(telegram_id: int, product_id: int, product_name: str,
    ...
    try:
        ...
        print(f"[DB DEBUG] user={telegram_id} product={product_id} name={product_name} qty={quantity} cost={total_cost} keys_count={len(keys) if keys else 0}", file=sys.stderr)
        return False
    finally:
        return_db(conn)
...
def upsert_product_override(product_id: int, price_usd: float = None,
                             stock: int = None, custom_price: bool = False) -> bool:
    """Set custom price/stock for a product. Pass None to keep existing value."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            # Check if override exists
            cur.execute(
                f"SELECT * FROM bot_product_overrides WHERE product_id = %s",
                (product_id,)
            )
            ...
            if existing:
                ...
                if custom_price is not None:
                    ...
                    params.append(1 if custom_price else 0)
                if updates:
                    params.append(product_id)
                    cur.execute(
                        f"UPDATE bot_product_overrides SET {', '.join(updates)} WHERE product_id = %s",
                        tuple(params)
                    )
            else:
                cur.execute(
                    "INSERT INTO bot_product_overrides (product_id, price_usd, stock, custom_price) "
                    "VALUES (%s, %s, %s, %s)",
                    (product_id,
                     price_usd if price_usd is not None else 0,
                     stock if stock is not None else 0,
                     1 if custom_price else 0)
                )
        conn.commit()
        return True
    except Exception as e:
        import sys
        print(f"[DB ERROR] upsert_product_override failed: {e}", file=sys.stderr)
        return False
    finally:
        return_db(conn)


def get_product_override(product_id: int) -> Optional[Dict]:
    """Get custom override for a product. Returns dict or None."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "SELECT * FROM bot_product_overrides WHERE product_id = %s",
                (product_id,)
            )
            return cur.fetchone()
    finally:
...
def get_all_product_overrides() -> List[Dict]:
    """Get all product overrides."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT * FROM bot_product_overrides ORDER BY product_id")
            return cur.fetchall()
    finally:
        return_db(conn)


def delete_product_override(product_id: int) -> bool:
    """Remove a product override (revert to API default)."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                "DELETE FROM bot_product_overrides WHERE product_id = %s",
                (product_id,)
            )
        conn.commit()
        return True
...
def get_most_purchased_products(limit: int = 20) -> List[Dict]:
    """Get top products by purchase count and revenue."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
...
def upsert_stock(product_id: int, product_name: str, category_name: str, stock: int, in_stock: bool, skip_alert: bool = False) -> bool:
    ...
    try:
        with conn.cursor() as cur:
            ...
            # First time seeing this product = no alert
            if not old:
                restocked = False
            elif skip_alert:
                restocked = False
            else:
                was_out = (old["last_stock"] == 0) or (old["last_in_stock"] == 0)
...
def get_proxy_price(pkg_id: int, days: int, hours: int) -> Optional[Dict]:
    """Get the admin-defined sell price for a proxy plan. Returns dict or None."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT * FROM {T_PROXY_PRICES} WHERE pkg_id = %s AND days = %s AND hours = %s",
                (pkg_id, days, hours)
            )
            return cur.fetchone()
    finally:
...
def upsert_proxy_price(pkg_id: int, days: int, hours: int,
                       base_price: float, sell_price: float, enabled: bool = True) -> bool:
    """Create or update a proxy plan's selling price."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"""INSERT INTO {T_PROXY_PRICES} (pkg_id, days, hours, base_price, sell_price, enabled)
                    VALUES (%s, %s, %s, %s, %s, %s)
                    ON DUPLICATE KEY UPDATE
                        base_price = VALUES(base_price),
                        sell_price = VALUES(sell_price),
                        enabled = VALUES(enabled)""",
                (pkg_id, days, hours, base_price, sell_price, 1 if enabled else 0)
            )
        conn.commit()
        return True
    except Exception:
...
def get_all_proxy_prices() -> List[Dict]:
    """Get all proxy pricing rows, newest updated first."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT * FROM {T_PROXY_PRICES} ORDER BY pkg_id, days, hours"
            )
            return cur.fetchall()
    finally:
        return_db(conn)
...
def get_enabled_proxy_prices(pkg_id: int = None) -> List[Dict]:
    """Get enabled proxy plans (optionally filtered by package)."""
    conn = get_db()
    try:
        with conn.cursor() as cur:
            if pkg_id is not None:
                cur.execute(
                    f"SELECT * FROM {T_PROXY_PRICES} WHERE enabled = 1 AND pkg_id = %s ORDER BY days, hours",
                    (pkg_id,)
                )
            else:
                cur.execute(
                    f"SELECT * FROM {T_PROXY_PRICES} WHERE enabled = 1 ORDER BY pkg_id, days, hours"
                )
            return cur.fetchall()
    finally:
        return_db(conn)
...
def create_proxy_order(user_id: int, country_name: str, package_name: str,
                       protocol: str, plan_days: int, plan_hours: int,
                       base_price: float, sell_price: float,
                       parent_proxy_id: int, proxy_details: str) -> bool:
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"""INSERT INTO {T_PROXY_ORDERS}
                    (user_id, country_name, package_name, protocol, plan_days, plan_hours,
...
def get_user_proxy_orders(user_id: int, limit: int = 10) -> List[Dict]:
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT * FROM {T_PROXY_ORDERS} WHERE user_id = %s ORDER BY created_at DESC LIMIT %s",
                (user_id, limit)
            )
            return cur.fetchall()
    finally:
...
def get_all_proxy_orders(limit: int = 100) -> List[Dict]:
    conn = get_db()
    try:
        with conn.cursor() as cur:
            cur.execute(
                f"SELECT * FROM {T_PROXY_ORDERS} ORDER BY created_at DESC LIMIT %s",
                (limit,)
            )
            return cur.fetchall()
    finally:
